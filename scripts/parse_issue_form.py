...
def validate_parsed_data(data: dict[str, str]) -> tuple[bool, list[str], list[str]]:
    ...
    required_fields = [
        'display_name', 'category', 'primary_link', 
        'author_name', 'author_link', 'description'
    ]
    ...
    # Validate category
    valid_categories = [
        'Workflows & Knowledge Guides', 'Tooling', 'Hooks', 
        'Slash-Commands', 'CLAUDE.md Files', 'Official Documentation'
    ]
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if row.get('Primary Link', '').lower() == primary_link:
                warnings.append(f"A resource with this primary link already exists: {row.get('Display Name')}")
