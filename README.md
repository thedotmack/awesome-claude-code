...
A push-to-talk speech transcription setup for Linux using a Python MCP server. Runs locally in Docker with no external API calls. Press the Right Ctrl key to record audio; once the key is released, the speech is transcribed into text and the transcript is sent to Claude running in a Tmux session.
